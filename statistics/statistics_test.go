...
func TestStatistics(t *testing.T) {
	t.Run("RecalculateAverage should recalculate the average", func(t *testing.T) {
		macdMock := NewMACDContainer(MACDParams{12, 26, 9}, []float64{})
		...
		tests := []struct {
			...
			{Statistics{StatisticsOptions{10}, []float64{}, 0, 0, macdMock}, 2, 2, 1},
			{Statistics{StatisticsOptions{10}, []float64{10}, 10, 1, macdMock}, 5, 7.5, 2},
			{Statistics{StatisticsOptions{10}, []float64{2, 20, 80}, 34, 3, macdMock}, 4, 26.5, 4},
			{Statistics{StatisticsOptions{2}, []float64{20, 80}, 50, 2, macdMock}, 40, 60, 2},
			{Statistics{StatisticsOptions{3}, []float64{20, 80}, 50, 2, macdMock}, 50, 50, 3},
		}
		...
		for index, tt := range tests {
			...
			if gotAverage != wantAverage {
				t.Errorf("#%v average: got %v want %v", index, gotAverage, wantAverage)
			}

			if gotNumberOfPoints != wantNumberOfPoints {
				t.Errorf("#%v number of points: got %v want %v", index, gotNumberOfPoints, wantNumberOfPoints)
			}
		}
	})
}
...
func TestEMA(t *testing.T) {
	t.Run("should return right EMA", func(t *testing.T) {
		got := float32(EMA(461.14, 12, 449.823954868254))
...
func TestMACDContainerAddPoint(t *testing.T) {
	t.Run("after adding 12 points should add first point to fast EMA", func(t *testing.T) {
		...
		mc := NewMACDContainer(params, holdPoints)
	...
	t.Run("after adding 26 points should add first point to slow EMA and MACD", func(t *testing.T) {
		...
		mc := NewMACDContainer(params, holdPoints, fastEma)
	...
	t.Run("after adding 35 points should add first point to lag EMA", func(t *testing.T) {
		...
		mc := NewMACDContainer(params, holdPoints, fastEma, slowEma, macd)
