...
func TestStatistics(t *testing.T) {
	t.Run("RecalculateAverage should recalculate the average", func(t *testing.T) {
		tests := []struct {
			...
			{Statistics{domain.StatisticsOptions{NumberOfPointsHold: 10}, []float64{}, 0, 0, 0}, 2, 2, 1},
			{Statistics{domain.StatisticsOptions{NumberOfPointsHold: 10}, []float64{10}, 10, 1, 0}, 5, 7.5, 2},
			{Statistics{domain.StatisticsOptions{NumberOfPointsHold: 10}, []float64{2, 20, 80}, 34, 3, 0}, 4, 26.5, 4},
			{Statistics{domain.StatisticsOptions{NumberOfPointsHold: 2}, []float64{20, 80}, 50, 2, 0}, 40, 60, 2},
			{Statistics{domain.StatisticsOptions{NumberOfPointsHold: 3}, []float64{20, 80}, 50, 2, 0}, 50, 50, 3},
		}

		for index, tt := range tests {
			tt.statistics.AddPoint(tt.valueToAdd)

			gotAverage := tt.statistics.GetAverage()
	...
	t.Run("RecalculateVariance should recalculate the variance", func(t *testing.T) {
		stats := Statistics{domain.StatisticsOptions{NumberOfPointsHold: 10}, []float64{}, 0, 0, 0}
	...
	t.Run("RecalculateVariance should recalculate the variance of last points", func(t *testing.T) {
		stats := Statistics{domain.StatisticsOptions{NumberOfPointsHold: 4}, []float64{}, 0, 0, 0}
